...
const channelResult = handleChannel(channel, newRecipient)
    ...
    if (score < 30) {
      level = "Low"
      advice =
        "No major warning signs detected, but still verify the request."
    } else if (score < 50) {
      level = "Medium"
      advice =
        "Pause and verify the request before sending money."
    } else if (score < 70) {
      level = "High"
      advice =
        "Do not send money until you have independently verified the request."
    } else {
      level = "Critical"
      advice =
        "Stop the payment and verify the request using a trusted contact method."
    }
    let intervention = ""
    ...
    res.json({ score, riskReasons, level, advice, intervention })
})
app.listen(3001, () => {
